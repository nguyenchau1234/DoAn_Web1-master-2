...
function addThongKe() {
    ...
    danhSachDonHang.forEach(donHang => {
        ...
        // Lặp qua từng Khóa Học trong đơn hàng
...
function openTab(nameTab) {
    ...
    switch(nameTab) {
        ...
        case 'Khóa Học': document.getElementsByClassName('sanpham')[0].style.display = 'block'; break;
...
function themSanPham() {
    ...
    for(var p of list_products) {
        if(p.masp == newSp.masp) {
            alert('Mã Khóa Học bị trùng !!');
            return false;
        }

        if(p.name == newSp.name) {
            alert('Tên Khóa Học bị trùng !!');
            return false;
        }
    }
    ...
    alert('Thêm Khóa Học "' + newSp.name + '" thành công.');
    document.getElementById('khungThemSanPham').style.transform = 'scale(0)';
}
function autoMaSanPham(company) {
    // hàm tự tạo mã cho Khóa Học mới
...
function addKhungSuaSanPham(masp) {
    ...
    var company = ["Java", "HTML", "JavaScript", "CSS", "PHP", "Python"];
...
function addTableKhachHang() {
    ...
    for (var i = 0; i < listUser.length; i++) {
        ...
        s += `<tr>
            ...
            <td style="width: 10%">
                <span class="password" data-password="` + u.pass + `">********</span>
                <button class="show-password" onclick="this.previousElementSibling.textContent = this.previousElementSibling.dataset.password; this.textContent = 'Hint'">Show</button>
            </td>
            <td style="width: 10%">
                <div class="tooltip">
                    <label class="switch">
